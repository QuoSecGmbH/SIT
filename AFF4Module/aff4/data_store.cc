...
AFF4Status MemoryDataStore::Get(const URN& urn, const URN& attribute,
    ...
    if (urn_it == store.end()) {
        //std::cout << "Get(): urn "<< urn.SerializeToString()<< " NOT_FOUND" << "\n";
        return NOT_FOUND;
    }
    ...
    if (attribute_itr == urn_it->second.end()) {
        ...
        //std::cout << "Get(): attribute NOT_FOUND" << "\n";
        return NOT_FOUND;
    }
...
std::string MemoryDataStore::GetAttributeValue(const URN& urn, const URN& attribute) {
    auto urn_it = store.find(urn.SerializeToString());
    ...
    return values[0].get()->SerializeToString();
}
