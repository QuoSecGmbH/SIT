...
AFF4Status VolumeGroup::GetStream(URN stream_urn, AFF4Flusher<AFF4Stream> &result) {
    ...
    if (STATUS_OK == resolver->Get(stream_urn, AFF4_TYPE, types)) {
        ...
        for (auto &type : types) {
            ...
            // For generic aff4:Image files, stored as zip members. Only possible in last
            // iteration and only if AFF4_IMAGE_TYPE was found in types
            counter++;
            if (type_str == AFF4_IMAGE_TYPE) imageFound = true;
            if (counter == (types.size()-1) && imageFound == true){
                URN owner;
                RETURN_IF_ERROR(resolver->Get(stream_urn, AFF4_STORED, owner));
    ...
    if (stream_urn == AFF4_IMAGESTREAM_ZERO) {
        ...
        std::cout << "GetStream(): stream_urn == AFF4_IMAGESTREAM_ZERO"<< "\n";
        return STATUS_OK;
    }
    if (stream_urn == AFF4_IMAGESTREAM_FF) {
        result = make_flusher<AFF4SymbolicStream>(resolver, stream_urn, 0xff);
        std::cout << "GetStream(): stream_urn == AFF4_IMAGESTREAM_FF" << "\n";
        return STATUS_OK;
    }
    ...
    if (stream_urn == AFF4_IMAGESTREAM_UNREADABLE) {
        ...
        std::cout << "GetStream(): stream_urn == AFF4_IMAGESTREAM_UNREADABLE" << "\n";
        return STATUS_OK;
    }
    ...
    for (int i = 0; i < 256; i++) {
        ...
        if (stream_urn == urn) {
            ...
            std::cout << "GetStream(): stream_urn == urn" << "\n";
            return STATUS_OK;
        }
    }
